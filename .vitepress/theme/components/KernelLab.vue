...
<template>
  <div class="lab">
    ...
    <div class="lab__stats">
      ...
      <div class="lab__stat"><b>{{ meta.sv }}</b><span>опорних векторів із 195</span></div>
    ...
    <p class="lab__note">
      Поставте C = 1 і γ = 0,1: схожість на відстані 1,0 дорівнює 0,90 — кожна
      точка впливає далеко навколо себе, межа виходить майже прямою, і модель
      недонавчається: 0,877 на навчальних і 0,848 на відкладених. Тепер γ = 50:
      схожість падає практично до нуля, межа розпадається на острівці навколо
      окремих спостережень, навчальна точність злітає до 0,990, а відкладена
      лишається 0,886 — перенавчання видно просто в цих двох числах. Далі
      зафіксуйте γ = 0,5 і посуньте C: він керує не радіусом, а ціною порушення
      відступу, тому змінює межу інакше — і кількість опорних векторів разом із
      нею, від 116 при C = 0,1 до 38 при C = 100. Саме тому ці два параметри
      підбирають спільно, сіткою, а не по черзі.
    </p>
    ...
    <p class="lab__note">
      ...
      gamma = 0,1 дає 0,933, C = 10 з gamma = 0,5 — найкращі 0,943, C = 1 з
      gamma = 0,5 — знову 0,933. Уздовж цієї смуги добуток C·gamma тримається
      приблизно сталим: більший радіус впливу компенсується суворішим штрафом.
      Клітинка C = 1, gamma = 0,5 — це рівно той рядок, який друкує код розділу
      про порівняння ядер: <code>gamma="scale"</code> на стандартизованих даних
      дорівнює 1/(2·1), тобто 0,5, і віджет дає ті самі 0,933 при 66
      опорних векторах із 195. Зелений кружок стоїть на 0,943, але дві сусідні клітинки дають 0,933, а
      цілий стовпчик при gamma = 1 — по 0,924, і різниця між ними менша за
      розкид між фолдами. Практичний висновок той самий, що в тексті розділу:
      сітку беруть логарифмічну, а всередині світлої смуги за третій знак не
      воюють. Клацніть по кутах карти: лівий верхній (мале gamma, велике C) дає
      майже пряму межу з 53 опорними векторами, правий нижній (велике gamma,
      мале C) — 0,495, тобто чисте вгадування, і всі 195 навчальних точок стають
      опорними: модель запам'ятала кожну й не навчилася нічого.
    </p>
  </div>
</template>
